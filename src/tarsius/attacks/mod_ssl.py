...
from tarsius.language.vulnerability import CRITICAL_LEVEL, HIGH_LEVEL, MEDIUM_LEVEL, INFO_LEVEL
from tarsius.utils.log import log_red, log_blue, log_green, log_orange
from tarsius.definitions.ssl import SslInformationFinding, SslVulnerabilityFinding
...
def sslscan_level_to_tarsius_level(security_level: str) -> int:
...
async def process_cipher_suites2(xml_file: str) -> AsyncIterator[Tuple[int, str]]:
    ...
    for protocol in protocol_versions:
        ...
        for security_level, ciphers in group_by_severity.items():
            ...
            yield sslscan_level_to_tarsius_level(security_level), message
