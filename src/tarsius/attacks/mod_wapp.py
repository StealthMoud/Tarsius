...
from tarsius.attacks.cve.checker import (
    CVEChecker, cvss_score_to_tarsius_level, CVE_DIRECTORY, SUPPORTED_SOFTWARES, is_cve_supported_software
)
from tarsius.core.controller.exceptions import InvalidOptionValue
from tarsius.definitions.vulnerable_software_version import VulnerableSoftwareFinding
from tarsius.utils.log import logging, log_blue, log_severity
...
from tarsius.wappalyzer.wappalyzer import Wappalyzer, ApplicationData, ApplicationDataException
from tarsius.definitions.fingerprint import SoftwareNameDisclosureFinding
from tarsius.definitions.fingerprint_webserver import WebServerVersionDisclosureFinding
...
SCRIPT = (
    "(tarsius_tests) => {\n"
    "  let tarsius_results = {};\n"
    "  for (var js_tech in tarsius_tests) {\n"
    "    let js_tech_results = [];\n"
    "    for (let i = 0; i < tarsius_tests[js_tech].length; i++) {\n"
    "      try {\n"
    "        js_tech_results.push([String(eval(tarsius_tests[js_tech][i])), tarsius_tests[js_tech][i]]);\n"
    "      } catch(tarsius_error) {\n"
    "        continue;\n"
    "      }\n"
    "    }\n"
    "    if (js_tech_results.length) {\n"
    "      tarsius_results[js_tech] = js_tech_results;\n"
    "    }\n"
    "  }\n"
    "  return tarsius_results;\n"
    "}"
)
...
class ModuleWapp(Attack):
            ...
            for cve in self._cve_checker.get_cves(software_name.lower(), version):
                ...
                for cvss_format in ["cvss4", "cvss3.1", "cvss3", "cvss2"]:
                    if cvss_format in cve:
                        severity = cvss_score_to_tarsius_level(cve[cvss_format])
    ...
    async def get_nvd_release_data(self) -> dict:
        ...
        release_url = "https://api.github.com/repos/tarsius-scanner/nvd-web-cves/releases/latest"
    ...
    async def download_latest_release_asset(self, software_name: str, dest_folder: str) -> bool:
        ...
        try:
            ...
            logging.error("Could not fetch tarsius-scanner/nvd-web-cves CVE index for %s: %s", software_name, exception)
            return False

        # Find the asset related to the software_name
        asset = next((a for a in latest_release.get("assets", []) if file_name in a["name"]), None)
        if asset is None:
            logging.error("No CVE data found for %s on tarsius-scanner/nvd-web-cves", software_name)
            return False
