...
from tarsius.definitions.csrf import CsrfFinding
...
class ModuleCsrf(Attack):
    ...
    async def is_csrf_verified(self, request: Request, response: Response):
        ...
        # Replace anti-csrf token value from form with "tarsius"
        mutated_post_params = [
            param if param[0] != self.csrf_string else [self.csrf_string, "tarsius"]
            for param in request.post_params
        ]

        # Replace anti-csrf token value from headers with "tarsius"
        special_headers = {}
        if response.headers and self.csrf_string in response.headers:
            special_headers[self.csrf_string] = "tarsius"

        # Replace anti-csrf token value from request headers with "tarsius"
