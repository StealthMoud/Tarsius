...
from tarsius.network.cookies import mitm_jar_to_cookiejar
from tarsius.network.response import Response
from tarsius.network.crawler import AsyncCrawler
from tarsius.network.classes import CrawlerConfiguration
from tarsius.network.async_stickycookie import AsyncStickyCookie
from tarsius.network.explorer import Explorer, EXCLUDED_MEDIA_EXTENSIONS
from tarsius.network.scope import Scope, wildcard_translate
from tarsius.utils.log import log_verbose, log_blue, logging
...
def mitm_to_tarsius_request(mitm_request: MitmRequest) -> Optional[Request]:
    post_params = []
    enctype = ""
...
class MitmFlowToTarsiusRequests:
    ...
    async def response(self, flow):
        ...
        if "text" in content_type or "json" in content_type or "html" in content_type or "xml" in content_type:
            request = mitm_to_tarsius_request(flow.request)
            if request is None:
                return
...
async def launch_proxy(
    ...
    # Finally here is our custom addon that will generate Tarsius Request and Response objects and push them to the queue
    master.addons.add(MitmFlowToTarsiusRequests(data_queue, headers, scope, drop_cookies))
    try:
        await master.run()
...
async def click_in_webpage(page, request: Request, wait_time: float, timeout: float):
    # Use XPath as CSS selectors lack support for combining nth-of-type with specific attributes
