...
from tarsius.attacks.attack import PayloadType, random_string
from tarsius.network import Response
...
def apply_attrname_context(context: Dict[str, str], payloads: List[Dict[str, str]], code: str) -> List[PayloadInfo]:
    ...
    if code == context["name"]:
        for payload_infos in payloads:
            if not payload_infos["close_tag"]:
                # Process non-closing tag payloads
                pass
            else:
                js_code = '>'
...
def generate_payloads(
        html_code: str, code: str, payload_file: str, external_endpoint: str = "http://tarsius3.ovh/"
) -> List[PayloadInfo]:
    """
...
if __name__ == "__main__":
    ...
    <body>
    <a href="injection">Hello World</a>
    <!-- injection -->
