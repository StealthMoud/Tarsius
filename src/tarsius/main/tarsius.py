...
def handle_special_commands(args: Namespace):
    # handling some special commands here
    if args.scope == "punk":
...
def validate_basic_args(args: Namespace):
    # check args and exit if something is wrong
    if args.tasks < 1:
...
async def tarsius_main():
    ...
    wap = Tarsius(base_request, scope=args.scope, session_dir=args.store_session, config_dir=args.store_config)

    await configure_tarsius_basic_settings(wap, args)
    ...
    try:
        ...
        logging.info("[*] Tarsius found %s URLs and forms during the scan", await wap.count_resources())
