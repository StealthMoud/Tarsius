...
from tarsius.network import jsoncookie
from tarsius.network.crawler import AsyncCrawler
from tarsius.network.classes import CrawlerConfiguration, HttpCredential, RawCredential
from tarsius.parsers.html_parser import Html
from tarsius.network import Request
from tarsius.network.auth import login_with_raw_data, async_fetch_login_page
...
async def getcookie_main(arguments):
    parser = argparse.ArgumentParser(description="Tarsius-getcookie: An utility to grab cookies from a webpage")
    ...
    parser.add_argument(
        '-c', '--cookie',
        help='Cookie file in Tarsius JSON format where cookies will be stored',
        required=True
    )
